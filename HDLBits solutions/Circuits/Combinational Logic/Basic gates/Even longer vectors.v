...
module top_module( 
   ...
   assign out_both   = in[98:0] & in[99:1];          // Check current bit and left neighbor (no wrap)
   assign out_any    = in[99:1] | in[98:0];          // Check current bit or right neighbor (no wrap)
   assign out_different = in ^ {in[0], in[99:1]};    // XOR with left neighbor (wrapping around using concatenation)
